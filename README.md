...
那么 monorepo 的 意思就是，在一个仓库中，可能会包含三个项目，这三个项目可能是独立的，可能会有相互依赖项。通常情况下，在项目根目录会有一个 `packages`，内部来存放三个项目。比如下面场景：
...
上面代码中，在大函数内部，每个函数内部都有一个 `num` 属性，这是每个函数私有的变量，但是在 `fun`内部，有一个可以提供所有函数使用的 `lodash` ，因为每个函数的内部并没有 `lodash`，所有就会向上一层进行查找，最终返回对应的值。
...
进入 `vue3-pnpm-monorepo` 文件夹，执行命令：
...
比如说，我的 `vue-demo1` 的项目中需要安装 `tyh-ui`，而其它的两个项目是不需要的，那么这样的话，就可以将 `tyh-ui` 单独安装到 `vue-demo1` 的项目中，而另外两个是必须要的，所以就没必要安装到全局，安装的方式有两种：
